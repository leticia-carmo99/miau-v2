...
export default function FormCPF2() {
      ...
      <View style={styles.cardContainer}>
        <ScrollView
          ...
          <Field label="Tipo de produto ou serviço oferecido" required>
            <TextInput
              style={styles.input}
              placeholder="Ex: pet shop, adoção, banho..."
              placeholderTextColor="#999"
              value={formData.tipoProduto}
              onChangeText={(text) => handleChange('tipoProduto', text)}
            />
          </Field>
          ...
          <Field label="Região de atuação (SP)" required>
            {[
              ...
              'Interior - Baixada Santista, Litoral Norte e Sul',
          ...
          <Field label="Possui espaço físico ou atende a domicílio?" required>
            {['Espaço físico', 'Atende a domicílio'].map((opcao) => (
              <TouchableOpacity
                key={opcao}
                style={styles.checkboxContainer}
                onPress={() => handleChange('localAtendimento', opcao)}>
                <View
                  style={[
                    styles.checkbox,
                    formData.localAtendimento === opcao &&
                      styles.checkboxSelected,
                  ]}
                />
                <Text
          ...
          <Field label="Faixa de preço dos serviços (opcional)">
            <TextInput
              style={styles.input}
              placeholder="Ex: R$ 50 - R$ 150"
              placeholderTextColor="#999"
              value={formData.faixaPreco}
              onChangeText={(text) => handleChange('faixaPreco', text)}
            />
          </Field>
